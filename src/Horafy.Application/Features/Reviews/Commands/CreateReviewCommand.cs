...
public sealed record CreateReviewCommand(
    Guid    BookingId,
    int     Stars,
    string? Comment) : IRequest<Result<CreateReviewResult>>;
...
internal sealed class CreateReviewCommandHandler(
    ...
    : IRequestHandler<CreateReviewCommand, Result<CreateReviewResult>>
{
    public async Task<Result<CreateReviewResult>> Handle(
        CreateReviewCommand request, CancellationToken cancellationToken)
    {
        if (!currentUserService.UserId.HasValue)
            return Result.Failure<CreateReviewResult>(new Error(
